...
@csrf_exempt
def upload_examples_csv(request, assistant_id):
...
def add_assistant(request, collective_name, channel_name):
    assistant_id = request.GET.get('assistant_id')
    assistant_name = request.GET.get('assistant_name')
    create_assistant(collective_name, channel_name, assistant_name, assistant_id)
...
def add_example(request, assistant_id):
